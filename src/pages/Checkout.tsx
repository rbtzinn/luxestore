...
type AddressField = 'street' | 'number' | 'complement' | 'neighborhood' | 'city' | 'state' | 'zip';
type PaymentMethod = 'credit_card' | 'pix' | 'boleto';

const checkoutSteps: Exclude<Step, 'complete'>[] = ['address', 'payment', 'review'];
const addressFields: AddressField[] = ['street', 'number', 'complement', 'neighborhood', 'city', 'state', 'zip'];
const paymentMethods: PaymentMethod[] = ['credit_card', 'pix', 'boleto'];
...
export default function Checkout() {
  ...
  const currentStepIndex = checkoutSteps.indexOf(step as Exclude<Step, 'complete'>);
  const stepLabels = {
    address: t('checkoutPage.address'),
    payment: t('checkoutPage.payment'),
    review: t('checkoutPage.review'),
  } satisfies Record<Exclude<Step, 'complete'>, string>;
  const addressPlaceholders = {
    street: t('checkoutPage.street'),
    number: t('checkoutPage.number'),
    complement: t('checkoutPage.complement'),
    neighborhood: t('checkoutPage.neighborhood'),
    city: t('checkoutPage.city'),
    state: t('checkoutPage.state'),
    zip: t('checkoutPage.zip'),
  } satisfies Record<AddressField, string>;
  const paymentLabels = {
    credit_card: t('checkoutPage.creditCard'),
    pix: 'PIX',
    boleto: 'Boleto',
    ...
    <div className="min-h-screen bg-background">
      <div className="container-premium py-12 max-w-4xl">
        ...
        <div className="flex items-center gap-2 mb-10 overflow-x-auto pb-1">
          {checkoutSteps.map((currentStep, index) => (
            <div key={currentStep} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-body font-semibold transition-colors ${
                step === currentStep ? 'bg-primary text-primary-foreground' : currentStepIndex > index ? 'bg-success text-success-foreground' : 'bg-secondary text-secondary-foreground'
              }`}>
                {currentStepIndex > index ? 'OK' : index + 1}
              </div>
              <span className="text-xs font-body text-muted-foreground hidden sm:inline">{stepLabels[currentStep]}</span>
              {index < checkoutSteps.length - 1 ? <div className="w-8 md:w-16 h-px bg-border" /> : null}
            </div>
          ))}
        </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {addressFields.map((field) => (
                    <input
                      ...
                      placeholder={addressPlaceholders[field]}
                      className={`input-premium ${field === 'street' ? 'col-span-2' : ''}`}
                    />
                  ))}
                </div>
                ...
                {paymentMethods.map((method) => (
                  <label key={method} className={`flex items-center gap-3 p-4 rounded-lg border cursor-pointer transition-colors ${payment === method ? 'border-foreground bg-secondary/50' : 'border-border'}`}>
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${payment === method ? 'border-foreground' : 'border-muted-foreground'}`}>
                      {payment === method ? <div className="w-2 h-2 rounded-full bg-foreground" /> : null}
                    </div>
                    <span className="text-sm font-body font-medium text-foreground capitalize">{paymentLabels[method]}</span>
                  </label>
                ))}
                {payment === 'credit_card' ? (
