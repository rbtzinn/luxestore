            ...
            <motion.div key={product.id} layout className="group">
              <Link to={`/products/${product.slug}`} className="block">
                <div className="relative aspect-square rounded-xl overflow-hidden bg-secondary mb-4">
                  ...
                  <button onClick={(e) => { e.preventDefault(); removeItem(product.id); }} className="absolute top-3 right-3 p-2 rounded-full bg-background/80 backdrop-blur-sm" aria-label={t('common.remove')}>
              ...
              <span className="text-sm font-body font-semibold text-foreground">{formatCurrency(product.sale_price ?? product.price, language)}</span>
            </motion.div>
          ))}
        </div>
      </div>
